...
from materdei_audit.controllers import SurgeryAuditor, ReportGenerator
from materdei_audit.models import ProtocolRulesRepository
from materdei_audit.config import OUTPUT_DIR, AUDIT_CONFIG, LOGGING_CONFIG
...
def main():
    ...
    try:
        ...
        logger.info(f"  ✓ {stats['total_rules']} regras carregadas")
        logger.info("")
        ...
        logger.info(f"  ✓ {count} cirurgias carregadas")
        logger.info("")
        
        # Executa auditoria
        logger.info("Executando auditoria...")
        results = auditor.audit_all_surgeries()
        logger.info(f"  ✓ {len(results)} cirurgias auditadas")
        logger.info("")
        ...
        if nao_conforme > 0:
            logger.warning("")
            logger.warning(f"⚠ {nao_conforme} casos não conformes detectados!")
            logger.warning("  Revise a aba 'Não Conformes' no Excel para detalhes.")
